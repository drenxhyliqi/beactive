...
- The brief: "Report — per file/hunk where relevant — every place the diff violates a documented
  standard. Cite the standard (file + the rule). Distinguish hard violations from judgement calls.
...
Do not merge or rerank findings — the two axes are deliberately separate (see Why two axes).
